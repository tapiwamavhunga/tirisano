...
@section('orders.content')
                    ...
                    <div class="col">
                        <h6 class="card-subtitle mb-2 text-muted">
                            <p class="card-text text-muted small ">
                               Created by <span class=""> {{$user->name }}</span> on {{$user->created_at}}</p>
                        </h6>
                    </div>
                </div>
            ...
            <div class="card-footer bg-white px-0 ">
                <div class="row">
                    ...
                    <div class=" col-md-auto ">
                        <a href="/profile" class="btn btn-outlined btn-black text-muted bg-transparent" data-wow-delay="0.7s"><img src="https://img.icons8.com/ios/50/000000/settings.png" width="19" height="19"> <small>MANAGE PROFILE</small></a>
                        ...
                        <a href="/orders" class="btn btn-outlined btn-black text-muted "><img src="https://img.icons8.com/metro/26/000000/more.png" class="mr-2 more" width="20" height="20"><small>MANAGE TRAVEL RECQUESTS</small></a>
                    </div>
    ...
    <thead role="rowgroup">
    <tr role="row">
                    <th role='columnheader'>Identifier ID</th>
    ...
    @foreach($orders as $order)
        <tr>                <td data-label="Lastname">{{Auth::user()->identifier_id}}</td>
                            <td data-label="Name">{{ $order->name ?: "(blank)" }}</td>
                            <td data-label="Lastname">{{ $order->lastname ?: "(blank)" }}</td>
                            <td data-label="Lastname">{{Auth::user()->email}}</td>
                            <td data-label="Lastname">{{Auth::user()->phone}}</td>
                            ...
                            <td data-label="Dateoftravel">{{ $order->isapproved ?: "(blank)" }}</td>
